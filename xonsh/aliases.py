    ...
    def eval_alias(self, value, seen_tokens=frozenset(), acc_args=()):
        ...
        if callable(value):
            if acc_args:  # Partial application
                ...
                def _alias(args, stdin=None, stdout=None, stderr=None, spec=None, stack=None):
                    args = list(acc_args) + args
                    return value(args, stdin=stdin, stdout=stdout, stderr=stderr, spec=spec, stack=stack)
