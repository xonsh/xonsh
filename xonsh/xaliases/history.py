...
class HistoryAlias(xcli.ArgParserAlias):
    ...
    def transfer(
        ...
        src = xhm.construct_history(backend=source, filename=source_file, gc=False)
        dest = xhm.construct_history(backend=target, filename=target_file, gc=False)
...
alias = HistoryAlias(threadable=True)
