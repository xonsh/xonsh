...
class Execer:
    ...
    def parse(self, input, ctx, mode="exec", filename=None, transform=True, user_names=None):
    ...
    def compile(
        ...
        tree = self.parse(
            input, ctx, mode=mode, filename=filename,
            transform=transform, user_names=user_names,
        )
        if tree is None:
            return (
