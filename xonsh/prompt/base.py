...
class ParsedTokens(tp.NamedTuple):
    ...
    def process(self) -> str:
        ...
        processor = XS.env.get(  # type: ignore
...
class PromptFormatter:
    ...
    def __call__(self, template=DEFAULT_PROMPT, fields=None, **kwargs) -> str:
        ...
        if fields is None:
            self.fields = XS.env["PROMPT_FIELDS"]  # type: ignore
        else:
            self.fields = fields

        # some quick tests
        if isinstance(fields, dict):
            pflds: PromptFields[PromptField] = PromptFields(XS, init=False)
            pflds.update(fields)
            self.fields = pflds
...
def is_template_string(template, PROMPT_FIELDS=None):
    ...
    if PROMPT_FIELDS is None:
        fmtter = XS.env.get("PROMPT_FIELDS", PROMPT_FIELDS)
    else:
